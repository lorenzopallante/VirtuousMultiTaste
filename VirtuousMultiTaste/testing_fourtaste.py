...
Example run:
    For two class:
        python3 testing_fourtaste.py testing_dataset.txt 
"""
import sys
import os
import time
...
def initLogging():
    ...
    logging.basicConfig(filename="{}/fourtaste_run_report_{}.log".format(os.getcwd(), todaystr),
...
def select_predictor(dataset_initial, features, models_zip, training_labels_filename, output_folder,
    ...
    for i in result[1]:
        ...
        predicted_new.append(index[0][0])

    if result[0]:
        create_labels_file(predicted_new, result[1], training_labels_filename, output_folder, user, jobid, pid)

        if os.path.isfile(output_folder + "preprocessed_data.txt"):
...
def create_labels_file(prediction, predictions_proba, training_labels_file, output_folder, user, jobid, pid):
    """
    ...
    :return:
    """
    predictions = transform_labels_to_alpha_multiclass(prediction, parse_training_labels(training_labels_file))
    try:
        ...
        with open(output_folder + "result_dominant_label.txt", "w") as result_labels_fid:
            i = 0
            for label in predictions:
                result_labels_fid.write(str(label) + "\t" + str(np.max(predictions_proba[i])) + "\n")
                i += 1
